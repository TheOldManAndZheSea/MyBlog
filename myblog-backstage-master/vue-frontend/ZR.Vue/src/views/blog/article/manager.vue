<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" v-hasPermi="['blog:article:add']" @click="handleAdd">发布文章</el-button>
      </el-col>
    ...
    <el-table :data="dataList" ref="table" border>
      ...
      <el-table-column prop="tags" label="标签" width="100" :show-overflow-tooltip="true"> 
        <template slot-scope="scope">
          <div v-if="scope.row.tags!=null">
            <el-tag size="mini" v-for="tag in scope.row.tags" :key="tag.id" disable-transitions>{{tag.tagName}}</el-tag>
          </div>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" width="160">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['blog:article:update']">编辑</el-button>
          <el-popconfirm title="确定删除吗？" @confirm="handleDelete(scope.row)" style="margin-left:10px">
            <el-button slot="reference" size="mini" type="text" icon="el-icon-delete" v-hasPermi="['blog:article:delete']">删除</el-button>
          </el-popconfirm>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import { listArticle, delArticle } from "@/api/blog/article.js";
...
export default {
  ...
  methods: {
    ...
    /** 新增按钮操作 */
    handleAdd() {
      this.$router.replace({ path: "publish" });
    },
    /** 删除按钮操作 */
    handleDelete(row) {
      delArticle(row.cid).then((res) => {
        if (res.code == 200) {
          ...
          this.msgError("删除失败："+res.msg);
        }
      });
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.$router.push({ path: "publish", query: { cid: row.cid } });
    },
    // 详情
    handleView(row) {
      this.open = true;
