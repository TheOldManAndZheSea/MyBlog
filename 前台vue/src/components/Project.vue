<template>
  <el-container>
    <el-row :gutter="10">
      <el-col :span="24">
        <h2 style="color: white; background-color: rgba(0, 0, 0, 0.15)">
          完整项目
        </h2>
      </el-col>
      <!--      <el-col :span="24">-->
      ...
      >
        <a :href="project.url" style="text-decoration: none" target="_blank">
          <el-card class="project">
            <el-image class="image" :src="project.picUrl" fit="fill">
...
export default {
  ...
  methods: {
    ...
    async getProjectList() {
      const { data: res } = await this.$blog.get("/Blog/TBlogproject/list", {
        params: { TypeName: "2" },
      });
      if (res.code === 200) {
