...
export default {
        ...
        if (val) {
          ...
          this.fileList = list.map((item) => {
            if (typeof item === "string") {
              ...
              // } else {
              item = { name: this.fileName?item:this.fileName, url: item };
              // }
            }
            return item;
          });
  ...
  methods: {
    ...
    handleBeforeUpload(file) {
      let isImg = false;
      //判断文件名的长度
      if(this.nameMaxLength!=0&&file.name.length>this.nameMaxLength)
      {
        this.msgError(`上传失败，原因:文件名的长度不能大于50!`);
        return false;
      }
