<template>
    <el-container>
        <el-timeline class="animate__animated animate__fadeInLeft">
            <el-timeline-item :color="essay.color" v-for="essay in essayList" :key="essay.id" :timestamp="essay.createTime| dataFormat" placement="top">
                <el-card style="letter-spacing: 1px;" :style="calcuteStyle(essay)">
                    ...
                    <!-- <el-image  :src="essay.image"></el-image> -->
                </el-card>
            </el-timeline-item>
        </el-timeline>
    </el-container>
...
export default {
    ...
    methods:{
        async getEssayList(){
            const {data: res} = await this.$blog.get('/blog/TBlogessays/list')
            debugger
            if (res.code === 200){
        ...
        calcuteStyle(essay){
            return 'border: 3px solid '+essay.color+'; background-color: rgba(255,255,255,0.9);box-shadow: 0 0 30px -10px '+essay.color
        }
    }
}
</script>
