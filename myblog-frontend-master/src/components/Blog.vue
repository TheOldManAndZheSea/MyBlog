  ...
  <el-container class="animate__animated animate__fadeIn ">
    <el-card style="background-color: rgba(255, 255, 255,1)" class="first-card">
      <div slot="header" class="total blog-info">
        <div class="user-info">
          <el-avatar size="small" :src="blog.author.avatar"></el-avatar>
          <a href="#" class="header">{{blog.author.nickName}}</a>
        </div>
        <div class="blog-date">
      ...
      <el-image class="blog-pic" :src="blog.first_picture"></el-image>
      <hr/>
      <h2 class="blog-title header">{{blog.title}}
        <el-tag v-if="blog.category&&blog.category.length>0" size="small" effect="plain" type="warning" style="font-weight: bold; font-size: small; margin-left: 20px">
          {{blog.category?blog.category.name:''}}
        </el-tag>
      </h2>
      <div class="typo m-padded-lr-responsive m-padded-tb-large" v-if="blog.fmt_type === 'markdown'"
           v-html="markdownToHtml" v-highlight></div>
           <div class="typo m-padded-lr-responsive m-padded-tb-large" v-else
           v-html="blog.content" v-highlight></div>
      <div class="tags">
        <div class="tag-item" v-for="tag in blog.tags" :key="tag.tag_id">
          <div class="sjx-outer">
        ...
        <el-popover
          ...
          <div class="give-money">
            <div class="give-money-item">
              <el-image :src="blogConfig.fundedWechartImg" title="微信支付"></el-image>
              <p>微信</p>
            </div>
            <div class="give-money-item">
              <el-image :src="blogConfig.fundedAlipayImg" title="支付宝支付"></el-image>
              <p>支付宝</p>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    async getBlogInfomation() {
      const {data: res} = await this.$blog.get(`/article/${this.$route.query.id}`)
      this.blog = res.data
